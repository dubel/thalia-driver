...
4. Arena half-extent is **168 m** (336 m square), same as Karaluch. Do not silently resize the lot.
...
| `src/game/Arena.ts` | empty lot, lighting, curb, grid |
| `src/game/terrain.ts` | gentle heightfield |
| `src/game/rig.ts` | `normalizeModel`, wheel pivots |
| `src/game/Car.ts` | arcade hull physics, sit-on-terrain |
| `src/game/camera.ts` | chase follow, same lag as Karaluch |
| `src/game/input.ts` | WASD, mouse yaw, space, R |
| `src/ui/hud.ts` | overlay + speedo |
